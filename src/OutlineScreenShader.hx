...
class OutlineScreenShader extends ScreenShader {
	static var SRC = {
		...
		function fragment() {
			...
			var current = raw.r;
			var above = heightTexture.get(input.uv - vec2(0, pad)).r;
			var below = heightTexture.get(input.uv - vec2(0, -pad)).r;
			var left = heightTexture.get(input.uv - vec2(pad, 0)).r;
			var right = heightTexture.get(input.uv - vec2(-pad, 0)).r;
